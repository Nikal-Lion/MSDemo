...
namespace MS.Services
{
    ...
    {
        public RoleService(IUnitOfWork<MSDbContext> unitOfWork, IMapper mapper, IdWorker idWorker, IClaimsAccessor claimsAccessor, IStringLocalizer localizer)
            : base(unitOfWork, mapper, idWorker, claimsAccessor, localizer)
        {
        ...
        public async Task<ExecuteResult> Delete(RoleViewModel viewModel)
        {
            ...
            await _unitOfWork.SaveChangesAsync(CancellationToken.None);//提交
            return result;
        }
